...
基于 4 个真实案例（某旗舰店）总结的标准化排查 skill。
...
1. **案例1** (`<conv_id_1>`): Agent 回复延迟 128 秒 → RPA 发送队列堵塞
2. **案例2** (<conv_id_2>...): 欢迎语延迟 66 秒 → RPA 处理队列排队
3. **案例3** (<conv_id_3>...): 欢迎语延迟 95 秒 → RPA 面板串台误判
4. **案例4** (<conv_id_4>...): 欢迎语延迟 195 秒 → RPA 面板未确认，3次重试

详细报告: 案例原始报告含客户数据，未随包分发（仅限内部留存）。
